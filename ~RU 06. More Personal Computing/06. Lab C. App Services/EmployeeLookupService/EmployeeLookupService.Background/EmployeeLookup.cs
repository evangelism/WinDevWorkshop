...
namespace EmployeeLookupService.Background
{
    public sealed class EmployeeLookup : IBackgroundTask
    {
        private BackgroundTaskDeferral deferral;
        private AppServiceConnection appServiceConnection;
        private Dictionary<string, string> employees = new Dictionary<string, string> { { "0", "John Smith" }, { "1", "Mary Echo" }, { "2", "Jane Doe" }, { "3", "Bob Harvey" } };
        ...
        private async void AppServiceConnection_RequestReceived(AppServiceConnection sender, AppServiceRequestReceivedEventArgs args)
        {
            ...
            foreach (var item in requestMessage)
            {
                ...
                {
                    responseMessage.Add(item.Value.ToString(), employees[item.Value.ToString()]);
                }
            }
